...
(defun remove-whitespace-enter () (interactive "*")
  (if (use-region-p)
...
(defun add-remove-tab-binding () (local-set-key (kbd "<tab>") '(lambda nil (interactive) (progn (dabbrev-expand 1) (insert " ")))))
(defun markdown-j-hook ()
  ...
  (local-set-key (kbd "C-S-o") 'move-to-done)
)
